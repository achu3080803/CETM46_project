...
movies_df<-read.csv("movies2.csv", encoding="UTF8")
...
p.als_imp <- as(predict(rec.ubcf, rr_matrix, type="topNList", n=10, ),"list")
cf_als_imp_movies <- NULL
for (i in 1:length(p.als_imp)){
  df_tmp <- data.frame(userId=c(i),movieId=as(p.als_imp[[1]],"vector"))
...
for (i in 1:length(p.popular)){
  df_tmp <- data.frame(userId=c(i),movieId=as(p.popular[[1]],"vector"))
